...
{
	out
		<< "Teller Setup: " << numLines << " Line(s), " << numTellers << " Teller(s)\n"
...
Teller::Teller(EventQueue *ln, int *nc, double *twt, double *mwt)
{
	line = ln;
	numCustomers = nc;
	totWaitTime = twt;
...
// TICKABLEQUEUE DEFS
TickableQueue::TickableQueue(EventQueue *evQ, double *tll, double *mll)
{
	eq = evQ;
	totLineLen = tll;
	maxLineLen = mll;
...
void TickableQueue::tick(int now)
{
	...
	{
		*totLineLen += double((now-lastTicked)*line->length());
	}
