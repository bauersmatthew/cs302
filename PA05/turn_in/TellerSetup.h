...
protected:
	...
	double totAvgWaitTime;
	double totMaxWaitTime;

	double totAvgLineLen;
	double totMaxLineLen;

	std::vector<double> totIdleTimePerTeller;
...
public:
	 ...
	 */
	Teller(EventQueue *ln, int *nc, double *twt, double *mwt);
...
public:
	 ...
	 */
	TickableQueue(EventQueue *evQ, double *tll, double *mll);
